...
function OCRResult({ result, onSaveSuccess }) {
  ...
  const handleSaveToHistory = async () => {
    try {
      ...
      setIsSaved(true);
      toast.success("Saved to your Medical History timeline!");

      if (onSaveSuccess) onSaveSuccess(newRecord);

      // Try syncing with backend API in background if online
      try {
        fetch("http://localhost:8000/api/medical-history/upload", {
          method: "POST",
            ...
            <div>
              <h3 className="text-xl font-bold text-white flex items-center gap-2">
                ...
                <Badge className="bg-emerald-500/20 text-emerald-300 border-emerald-500/30 text-[10px]">
                  OCR Accuracy: {data.confidence || 95}%
                </Badge>
              </h3>
